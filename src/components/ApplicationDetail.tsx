...
// STAGES stores the vivid dot/decoration color as e.g. 'var(--color-stage-offer)' —
// derive the darker, text-contrast-safe sibling token for use as text color.
function stageTextColor(stageColorVar: string) {
  return stageColorVar.replace(/\)$/, '-text)')
}
...
function FitBadge({ score }: { score: number }) {
  ...
  return (
    <span
      ...
      className="shrink-0 inline-flex items-baseline gap-0.5 text-[12px] font-semibold px-1.5 py-0.5 rounded"
      style={{
        color,
...
// Only the fields the overview section owns — notes/jdText/archived/extraction status are handled separately
type OverviewDraft = Omit<Application, 'id' | 'createdAt' | 'notes' | 'jdText' | 'archived' | 'extracting' | 'extractionFailed' | 'pending'>
...
export function ApplicationDetail({ application, onClose, onUpdate, onDelete, onToast }: Props) {
  ...
  // Captured once during the very first render (before showModal() ever runs),
  // so it's immune to StrictMode's dev-only double-invoke of mount effects —
  // capturing this inside the effect instead would sometimes read back the
  // dialog's own just-focused child on the second invocation.
  ...
  // Pick up the background-filled job description once extraction finishes
  // (the modal may still be open if the user reopened a pending application)
  const wasExtracting = useRef(application.extracting)
  useEffect(() => {
    if (wasExtracting.current && !application.extracting) {
      setLocalJdText(application.jdText)
    }
    wasExtracting.current = application.extracting
  }, [application.extracting, application.jdText])
  ...
  function toggleArchived() {
    onUpdate(application.id, { archived: !application.archived })
    onToast?.(application.archived ? `${application.company} restored` : `${application.company} archived`, 'success')
  }
  ...
  return (
    <dialog
      ...
      className="w-full h-full sm:h-auto sm:max-h-[90vh] sm:max-w-4xl bg-canvas rounded-none sm:rounded-card border-0 sm:border sm:border-border shadow-card-drag flex flex-col outline-none"
    >
      <div onClick={e => e.stopPropagation()} className="flex flex-col flex-1 min-h-0">

        {/* Header — no edit controls here */}
        <div className="flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-3 sm:py-4 border-b border-border shrink-0">
          ...
          <span
            ...
            style={{
              ...
              color: stageTextColor(activeStage.color),
            }}
          >
            <span
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 flex flex-col gap-5">
          ...
          <section className="bg-card border border-border rounded-card p-4 sm:p-5">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-2">
                    ...
                    {!overviewEditing && analysis?.fitScore !== undefined && (
                      <FitBadge score={analysis.fitScore} />
                    )}
                  </div>
                  {overviewEditing ? (
                    ...
                    <span className="flex items-center gap-1.5 text-[12px] text-ink-muted/60 italic">
                ...
                {overviewEditing ? (
                  ...
                  <div className="flex flex-col gap-3">
                    <p className="text-[13px] text-ink-muted/70 italic">
                      Fetching job details from the posting…
                    </p>
                    <div className="grid grid-cols-2 gap-x-8 gap-y-5" aria-hidden="true">
                      {[0, 1, 2, 3].map(i => (
                        <div key={i} className="flex flex-col gap-1.5">
                          <div className="h-2.5 rounded bg-column animate-pulse w-1/3" />
                          <div className="h-3.5 rounded bg-column animate-pulse w-2/3" />
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    {application.extractionFailed && (
                      <p className="text-[12px] text-stage-rejected">
                        Couldn't automatically fetch this job posting — fill in the details below.
                      </p>
                    )}
                    <div className="grid grid-cols-2 gap-x-4 sm:gap-x-8 gap-y-4 sm:gap-y-5">
                      ...
                      {(application.salary || formattedDate) && (
          ...
          {/* Row 2: Job Description | Notes — side by side, same height */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 items-stretch">
            <section className="bg-card border border-border rounded-card p-4 sm:p-5 flex flex-col">
              ...
              <textarea
                ...
                disabled={application.extracting}
                placeholder={
                  application.extracting
                    ? 'Fetching from the job posting…'
                    : 'Paste the job description here to enable AI analysis…'
                }
                className={`${inputCls} resize-none flex-1 min-h-[120px] disabled:opacity-50 disabled:cursor-not-allowed`}
              />
            </section>
            <section className="bg-card border border-border rounded-card p-4 sm:p-5 flex flex-col">
              <h3 className="text-[11px] font-semibold text-ink-muted uppercase tracking-widest mb-3">
          ...
          <div className="bg-card border border-border rounded-card flex flex-col">
            ...
            <div className="flex flex-col sm:flex-row items-stretch">
              <div className="w-full sm:w-[320px] shrink-0 p-4 sm:p-5 border-b sm:border-b-0 sm:border-r border-border">
                <PositioningPanel
            ...
            <div className="flex flex-col sm:flex-row border-t border-border relative">
              ...
              <button
                ...
                className="flex-1 px-4 py-3 text-[13px] font-medium text-ink-muted hover:text-ink hover:bg-column transition-colors border-b sm:border-b-0 sm:border-r border-border disabled:opacity-40 disabled:cursor-not-allowed focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-inset"
              >
          ...
          {analyzing && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {(['Strengths', 'Gaps', 'Keywords', 'Talking points'] as const).map(label => (
                <section key={label} className="bg-card border border-border rounded-card p-4 sm:p-5">
          ...
          {!analyzing && analysis && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <section className="sm:col-span-2 bg-card border border-border rounded-card p-4 sm:p-5">
                <h3 className="text-[11px] font-semibold text-ink-muted uppercase tracking-widest mb-3">
                  Talking points
                </h3>
          ...
          {/* Keywords — least critical section, kept just above history */}
          {!analyzing && analysis && (
            <section className="bg-card border border-border rounded-card p-4 sm:p-5">
              <h3 className="text-[11px] font-semibold text-ink-muted uppercase tracking-widest mb-3">
                Keywords
          ...
          {/* Archive / Delete — bottom of modal */}
          <div className="flex justify-start items-center gap-3 pt-2 pb-1">
            <button
              type="button"
              onClick={toggleArchived}
              className="px-3 py-1.5 rounded-button border border-border text-ink-muted text-[13px] font-medium hover:text-ink hover:bg-column transition-colors focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-1"
            >
