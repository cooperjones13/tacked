...
export function ResumeDrawer({ open, onClose }: Props) {
  if (!open) return null
  return <ResumeModal onClose={onClose} />
}
...
function ResumeModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    function handleCancel(e: Event) {
      e.preventDefault()
      // Only close the resume modal if the PDF preview isn't open.
      // When preview is open it sits on top and receives ESC first.
      onCloseRef.current()
    }
    dialog.addEventListener('cancel', handleCancel)
    return () => dialog.removeEventListener('cancel', handleCancel)
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              {resumes?.map(resume => (
                <div
                  key={resume._id}
                  className="flex items-start gap-3 bg-card border border-border rounded-card px-4 py-3"
                >
                  <div
